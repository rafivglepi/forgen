...
| ------- | ---------------------------------- | -------------- |
| `macro` | The Forgen macro                   | `forgen`       |
| `api`   | Internal API + plugin API          | `forgen-api`   |
| `cli`   | Build tool                         | `cargo-forgen` |
| `test`  | Test crate for plugins and the CLI |                |
...
This is a temporary section to keep the old, bloated README.md, which may have outdated information, but can also provide more technical details. (Honestly I just want to make sure I'm not removing some important information I forgot to update)
...
The `forgen-tests/` directory contains a sample project that demonstrates:
...
```bash
# From the workspace root, local types only
cargo run --bin cargo-forgen --manifest-path forgen-cli/Cargo.toml -- forgen-tests/Cargo.toml

# Or install forgen-cli first, then run from anywhere
cargo install --path forgen-cli
cargo-forgen forgen-tests/Cargo.toml

# With external libraries
cargo-forgen forgen-tests/Cargo.toml --include-external
```
